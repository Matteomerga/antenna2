...
# costanti di conversione (corrente , corrente motore , tensione)
kv = 0.00606203   # conversione tensione 
ki = -0.0073982   # conversione corrente 
kim = 0.0185   # conversione corrente motore
#raggio_ruota = 0.265 
raggio_ruota = 0.01
circonferennza = 2 * np.pi * raggio_ruota
numero_fori = 2


# --- CONFIGURAZIONE DELLA PORTA SERIALE ---
#PORTA_SERIAL = "COM4"                     #  potrebbe esserci errore sulla porta, verificare su arduino IDE la porta
PORTA_SERIAL = "/dev/cu.usbmodem101"  # per mac, verificare su arduino IDE la porta
#PORTA_SERIAL = "/dev/tty.usbserial-A5069RR4"
BAUD_RATE = 115200
ser = serial.Serial(PORTA_SERIAL, BAUD_RATE, timeout=1)
FORMATO_DATI = "<hhhhhiiih"
...
pacchetti_da_ricevere = 1          # eventualmente modificare insieme ai pacchetti inviati da arduino
...
def update_dashboard():
    ...
    labels['currMotor'].config(text=f"CorrenteMotore: {dashboard_currMotor:.2f} A")
...
def serial_reader(ser, stop_event, root, labels):
    ...
    with open(file_path, mode='a', newline='') as csvfile:
        ...
        if is_new:
            writer.writerow(["micro", "voltage","motorCurrent", "speed", "lat", "lon", "extra1", "extra2", "extra3", "current", "gps speed", "voltage_raw", "motorCurrent_raw", "current_raw", "tick_count"])
            csvfile.flush()
        ...
        try:
            ...
            while not started and not stop_event.is_set():
                ...
                while len(buffer) >= PACCHETTO_SIZE:
                    ...
                    try:
                        velocita_gps, voltage_raw, current_raw, currentMotor_raw, tick_count, lat, lon, micros, verifica = struct.unpack(FORMATO_DATI, pacchetto)
                    except struct.error:
                        # scarta 1 byte e riprova
                        buffer = buffer[1:]
                        continue

                    if int(tick_count + voltage_raw + current_raw + micros % 10000) == verifica:
                        started = True
            ...
            # lettura continua
            global dashboard_vel, dashboard_volt, dashboard_curr, dashboard_currMotor, dashboard_time, pacchetti_ricevuti
            while not stop_event.is_set():
                data = ser.read(PACCHETTO_SIZE)
                if len(data) != PACCHETTO_SIZE:
                    continue
                velocita_gps, voltage_raw, current_raw, currentMotor_raw, tick_count, lat, lon, micros, verifica = struct.unpack(FORMATO_DATI, data)
                pacchetti_ricevuti += 1
                

                #conversione parametri 

                voltage = voltage_raw * kv
                current = current_raw * ki
                currentMotor = currentMotor_raw * kim
                velocita = (circonferennza / numero_fori) * tick_count / (micros - previous_micros) * 1000000 * 3.6
                previous_micros = micros
                velocita_gps = velocita_gps / 100.0

                # scrivo su CSV
                writer.writerow([micros, voltage, currentMotor, velocita, lat, lon, 0, 0, 0, current, velocita_gps, voltage_raw, currentMotor_raw, current_raw, tick_count])
                csvfile.flush()
